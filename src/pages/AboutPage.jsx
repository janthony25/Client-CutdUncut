  ...
  const features = [
    {
      ...
      title: "Expert Team",
      description: "Our skilled stylists bring years of experience and continual education in the latest techniques."
    },
    {
      icon: <Scissors className="h-6 w-6 text-white" />,
      title: "Quality Products",
      description: "We use only premium hair products that deliver exceptional results while caring for your hair."
    },
    {
      icon: <Clock className="h-6 w-6 text-white" />,
      title: "Personalized Service",
      description: "Every client receives a customized consultation to ensure your unique needs are met."
    },
    {
      icon: <Award className="h-6 w-6 text-white" />,
      title: "Award Winning",
      description: "Recognized for excellence in styling, customer service, and innovation in the industry."
    }
  ];
    ...
    <div className="bg-black text-white pt-20">
      ...
      <section className="py-20" ref={storyRef}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h1 className="text-5xl md:text-6xl font-serif mb-4">About GUPIT</h1>
          ...
          >
            <motion.div variants={itemVariants} className="order-2 md:order-1">
              ...
              <p className="text-gray-300 mb-4">
                Founded in 2015, Cut d Uncut began with a simple vision: to create a salon experience that combines technical excellence with personalized care. What started as a small studio has grown into a destination for those seeking exceptional hair services.
              </p>
              <p className="text-gray-300 mb-4">
                Our name "Cut d Uncut" represents our philosophy of revealing your true beauty through our artistry. We believe that the perfect cut or color doesn't just change your hair – it enhances your natural features and boosts your confidence.
              </p>
              <p className="text-gray-300">
                Today, we're proud to be a team of passionate stylists dedicated to helping clients look and feel their best. Whether you're seeking a subtle trim or a bold transformation, we're here to bring your vision to life.
              </p>
            </motion.div>
            
            <motion.div variants={itemVariants} className="order-1 md:order-2 grid grid-cols-2 gap-4">
              {features.map((feature, idx) => (
                <div key={idx} className="bg-gray-900/50 backdrop-blur-sm p-4 rounded-lg border border-gray-800">
            ...
            {workItems.map((work, index) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                  <h3 className="text-base sm:text-lg font-medium text-white mb-1">{work.title}</h3>
                  <p className="text-gray-300 text-xs sm:text-sm">{work.description}</p>
                </div>
              </motion.div>
            ))}
          </motion.div>
      ...
      <section className="py-20 bg-gray-950" ref={testimonialsRef}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {[0, 3, 6, 9].map((index) => (
              <button
                ...
                className={`h-3 w-3 rounded-full transition-colors duration-300 ${
                  Math.floor(currentTestimonial / 3) === Math.floor(index / 3) ? 'bg-white' : 'bg-gray-600 hover:bg-gray-500'
                }`}
                aria-label={`View testimonial group ${Math.floor(index / 3) + 1}`}
              />
            ))}
          </div>
          ...
          <motion.div 
            ...
            animate={isTestimonialsInView ? "visible" : "hidden"}
          >
            <button className="px-8 py-3 bg-white text-black border-2 border-white hover:bg-black hover:text-white transition-all duration-300 text-lg tracking-wider uppercase shadow-lg">
              Book Appointment
            </button>
          </motion.div>
        </div>
